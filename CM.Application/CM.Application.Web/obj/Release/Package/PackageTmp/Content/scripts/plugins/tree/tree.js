...
(function ($) {
    ...
    //选中treeview节点 node:{id,text,value}
    $.fn.sxSetTreeViewNode = function (node) {
        var divid = $(this).attr('id').trim();
        if (node.id) 
            $("#itemTree_"+node.id).trigger("click");        
        if (node.text) 
            $("#"+divid+" li.bbit-tree-node>div[data-text='"+node.text+"']").trigger("click");
        if (node.value) 
            $("#" + divid + " li.bbit-tree-node>div[data-value='" + node.value + "']").trigger("click");
    }
    $.fn.treeview = function (settings) {
        ...
        var dfop =
            {
                ...
                url: false,
                /**
                 *  shouxin 2019-05-27 数据加载成功后执行函数
                 *  参数 data：所有数据
                 */
                onLoadSuccess: null,
                param: null,
        ...
        //shouxin 2019-5-27 加载并创建完成后回调
        if (dfop.onLoadSuccess) {
            dfop.onLoadSuccess(dfop.data);
        }
        //region 
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            //shouxin 2019-05-27 add text and value 
            ht.push("<div id='", id, "_", nid, "' data-text='",nd.text,"' data-value='",nd.value,"' tpath='", path, "' unselectable='on' title='", title, "'");
            var cs = [];
