    ...
    public class DocumentFileController : MvcControllerBase
    {
        ...
        public ActionResult GetDataGridJsonV1(Pagination pagination, string queryJson, string itemId)
        {
            ...
            if (!queryParam["condition"].IsEmpty() && !queryParam["keyword"].IsEmpty())
            {
                ...
                lisWhere.Add(new LeaRun.Util.WebControl.Conditions() { Key = condition, Value=keyword, IsLike=true });
            }
            if(!string.IsNullOrEmpty(itemId))
            {
                lisWhere.Add(new LeaRun.Util.WebControl.Conditions() { Key = "cinvClassCode", Value = itemId, IsLike = true });
            }
