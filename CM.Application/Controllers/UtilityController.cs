...
namespace CM.Application.Web.Controllers
{
    ...
    public class UtilityController : Controller
    {
        ...
        /// <summary>
        /// Excel数据列表查询方式:splitPage
        /// </summary>
        /// <returns></returns>
        public ActionResult ExcelExportForm_Comm()
        {
            return View();
        }
        /// <summary>
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult ExcelExportForm_CommV1()
        {
            return View();
        }
        /// <summary>
        ...
        /// <returns></returns>
        [ValidateInput(false)]
        public void ExecuteExportExcel_EasyUI(string columnJson, string rowJson, string exportField, string filename)
        {
            //设置导出格式
            ExcelConfig excelconfig = new ExcelConfig();
            excelconfig.Title = Server.UrlDecode(filename);
            ...
            excelconfig.ColumnEntity = new List<ColumnEntity>();
            //表头
            List<GridColumnModelEasyUI> columnData = columnJson.ToList<GridColumnModelEasyUI>();
            //行数据
            DataTable rowData = rowJson.ToTable();
            //写入Excel表头
            string[] fieldInfo = exportField.Split(',');
            foreach (string item in fieldInfo)
            {
                var list = columnData.FindAll(t => t.field == item);
                foreach (GridColumnModelEasyUI gridcolumnmodel in list)
                {
                    if (gridcolumnmodel.hidden == false && gridcolumnmodel.field != null)
                    {
                        string align = gridcolumnmodel.align;
                        excelconfig.ColumnEntity.Add(new ColumnEntity()
                        {
                            Column = gridcolumnmodel.field,
                            ExcelColumn = gridcolumnmodel.title,
                            //Width = gridcolumnmodel.width,
                            Alignment = gridcolumnmodel.align,
                        });
                    }
