...
namespace CM.Application.Web
{
    ...
    public class HandlerLoginAttribute : AuthorizeAttribute
    {
        ...
        public override void OnAuthorization(AuthorizationContext filterContext)
        {
            ...
            {
                WebHelper.WriteCookie("CM_login_error", "Overdue");//登录已超时,请重新登录
            ...
            if (OnLine == 0)
            {
                ...
                if (!checkOnLine)
                {
                    WebHelper.WriteCookie("CM_login_error", "OnLine");//您的帐号已在其它地方登录,请重新登录
                    filterContext.Result = new RedirectResult("~/Login/Default");
                    return;
                }
            }
            ...
            {
                WebHelper.WriteCookie("CM_login_error", "-1");//缓存已超时,请重新登录
                //filterContext.Result = new RedirectResult("~/Login/Default");
                return;
            }
        }
