...
(function ($) {
    ...
    $.CMtab = {
        ...
        scrollTabRight: function () {
            ...
            var tabOuterWidth = $.CMtab.calSumWidth($(".lea-tabs").children().not(".menuTabs"));
        ...
        init: function () {
            $('.menuTabs').on('click', '.menuTab .tab_close', $.CMtab.closeTab);
            $('.menuTabs').on('click', '.menuTab', $.CMtab.activeTab);

            $('.tabLeft').on('click', $.CMtab.scrollTabLeft);
            $('.tabRight').on('click', $.CMtab.scrollTabRight);
            $('.tabReload').on('click', $.CMtab.refreshTab);
    ...
    $.CMindex = {
        ...
        loadMenu: function (isInit) {
            var flag = false;
            var topMenuWidth = $('.lea-Head').width() - $.CMtab.calSumWidth($(".lea-Head").children().not(".left-bar"));
            ...
            $.each(data, function (i) {
                ...
                if (row.ParentId == "0") {
                    ...
                    _itemHtml += '</a>';
                    var childNodes = $.CMindex.jsonWhere(data, function (v) { return v.ParentId == row.ModuleId });
                    if (childNodes.length > 0) {
                        _itemHtml += '<div class="popover-menu"><div class="arrow"><em></em><span></span></div><ul class="treeview-menu">';
                        $.each(childNodes, function (i) {
                            var subrow = childNodes[i];
                            var subchildNodes = $.CMindex.jsonWhere(data, function (v) { return v.ParentId == subrow.ModuleId });
                            _itemHtml += '<li>';
            ...
            if (isInit || flag) {
                ...
                $('.menuiframe').unbind();
                $('.menuiframe').on('click', $.CMtab.addTab);
