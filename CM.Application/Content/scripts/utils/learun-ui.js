    ...
    function loadComboBoxView(json, searchValue, m) {
        if (json.length > 0) {
            ...
            $.each(json, function (i) {
                ...
                if (searchValue != undefined) {
                    if (row[m.text].indexOf(searchValue) != -1) {
                        $_html.append('<li data-index="'+i+'" data-value="' + row[options.id] + '" title="' + title + '">' + row[options.text] + '</li>');
                    }
                }
                else {
                    $_html.append('<li data-index="' + i + '" data-value="' + row[options.id] + '" title="' + title + '">' + row[options.text] + '</li>');
            ...
            $option.find('li').click(function (e) {
                ...
                $select.find('.ui-select-text').html(data_text).css('color', '#000');
                $option.slideUp(150);
                //shouxin//增加绑定数据行参数 源代码：$select.trigger("change")
                $select.trigger("change", [json[parseInt($(this).attr("data-index"))]]);
                e.stopPropagation();
            }).hover(function (e) {
                if (!$(this).hasClass('liactive')) {
                    $(this).toggleClass('on');
...
$.fn.getQueryParmsAll = function getQueryParmsAll() {
    var queryParms = new Object();
    var Conditions = [];
    var TimeString = [];
    var item = $(this).find("form").serializeArray();
    for (var i = 0; i < item.length; i++) {
        if (item[i].name.indexOf("TimeString") >= 0) {
            if (!eval(item[i].name.split('.')[0])) {
                TimeString[TimeString.length] = new Object();
            };
            eval(item[i].name + " ='" + item[i].value + "'");
        }
        else {
            if (!eval(item[i].name.split('.')[0])) {
                Conditions[Conditions.length] = new Object();
            };
            //对数组进行赋值
            eval(item[i].name + " ='" + item[i].value + "'");
        }
    }
    queryParms.Conditions = Conditions;
    queryParms.ConditionsTime = TimeString;
    return queryParms;
}

$.fn.setQueryParmsAll = function setQueryParmsAll() {
    //console.log("where ing");
    var queryParms = new Object();
    var Conditions = [];
    var TimeString = [];
    var item = $("#searchbar form").serializeArray();
    ...
    queryParms.Conditions = Conditions;
    queryParms.ConditionsTime = TimeString;
    //console.info(JSON.stringify(conditions)); 
    return queryParms;
};

$.fn.setQueryParms = function setQueryParms() {
    var queryParms = new Object();
    var Conditions = [];
...
$.fn.SetWebControls = function (data) {
    ...
    for (var key in data) {
        ...
        if (id.attr('id')) {
            ...
            if (id.hasClass("datetime10")) {
                type = "datetime10";
            }
            if (id.hasClass("datetime18")) {
                type = "datetime18";
            }
...
$.fn.authorizeButton = function (arg) {
    if (arg && arg.toString() == "System") return;
    var $element = $(this);
...
$.fn.authorizeColModelV1 = function () {
    var $element = $(this);
    var columnModel = $element.jqGrid('getGridParam', 'colModel');
    ...
    $.each(columnModel, function (i) {
        ...
        if (columnModel[i].name == "autoID") {

            $element.hideCol("autoID");
        }
    });
    var moduleId = tabiframeId().substr(6);
    var data = top.authorizeColumnData[moduleId];
...
//平板单页无框架
dialogOpen_DanYe = function (options) {
    Loading(true);
    var defaults = {
        id: null,
        title: '系统窗口',
...
$.fn.easyUiGridRowValue = function (code) {
    var $datagrid = $(this);
    var json = [];
