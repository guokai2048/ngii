    ...
    tablist = {
        newTab: function (item) {
            ...
            if (flag) {
                ...
                $.CMtab.scrollToTab($('.menuTab.active'));
            }
        }
    }
    $.CMtab = {
        ...
        scrollTabRight: function () {
            ...
            var tabOuterWidth = $.CMtab.calSumWidth($(".content-tabs").children().not(".menuTabs"));
            ...
            if ($(".page-tabs-content").width() < visibleWidth) {
                ...
                scrollVal = $.CMtab.calSumWidth($(tabElement).prevAll());
                if (scrollVal > 0) {
                    $('.page-tabs-content').animate({
        ...
        scrollTabLeft: function () {
            ...
            if ($(".page-tabs-content").width() < visibleWidth) {
                ...
                offsetVal = 0;
                if ($.CMtab.calSumWidth($(tabElement).prevAll()) > visibleWidth) {
                    while ((offsetVal + $(tabElement).outerWidth(true)) < (visibleWidth) && tabElement.length > 0) {
                        offsetVal += $(tabElement).outerWidth(true);
        ...
        scrollToTab: function (element) {
            var marginLeftVal = $.CMtab.calSumWidth($(element).prevAll()), marginRightVal = $.CMtab.calSumWidth($(element).nextAll());
            var tabOuterWidth = $.CMtab.calSumWidth($(".content-tabs").children().not(".menuTabs"));
            var visibleWidth = $(".content-tabs").outerWidth(true) - tabOuterWidth;
            var scrollVal = 0;
        ...
        init: function () {
            $('.menuItem').on('click', top.$.CMtab.addTab);
            $('.menuTabs').on('click', '.menuTab i', top.$.CMtab.closeTab);
            $('.menuTabs').on('click', '.menuTab', top.$.CMtab.activeTab);
            $('.tabLeft').on('click', top.$.CMtab.scrollTabLeft);
            $('.tabRight').on('click', top.$.CMtab.scrollTabRight);
            $('.tabReload').on('click', top.$.CMtab.refreshTab);
            ...
            $('.tabCloseOther').on('click', $.CMtab.closeOtherTabs);
            $('.fullscreen').on('click', function () {
                if (!$(this).attr('fullscreen')) {
                    $(this).attr('fullscreen', 'true');
                    $.CMtab.requestFullScreen();
                } else {
                    $(this).removeAttr('fullscreen')
                    $.CMtab.exitFullscreen();
                }
            });
        }
    };
    $.CMindex = {
        ...
        loadMenu: function () {
            ...
            $ul.find("li").click(function () {
                ...
                var childNodes = $.CMindex.jsonWhere(data, function (v) { return v.ParentId == dataId });
                var _childNodeshtml = "";
                $.each(childNodes, function (i) {
                    var row = childNodes[i];
                    var subchildNodes = $.CMindex.jsonWhere(data, function (v) { return v.ParentId == row.ModuleId });
